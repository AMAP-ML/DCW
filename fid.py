...
@click.option('--scaler', 'scaler',      help='scaler ', metavar='FLOAT',    type=float, required=True)
...
def calc(image_path, ref_path, scaler, num_expected, seed, batch):
    ...
    if dist.get_rank() == 0:
        ...
        # Extract parameter from image_path
        param = os.path.basename(image_path).split('_')[-1]
        save_path = f'fid/steps_35/low_sig_high_sig.csv'
        save_fid_to_csv(save_path, scaler, fid)
        # Write to log file
        # log_file = 'tui.log'
        # with open(log_file, 'a', newline='') as f:
        #     writer = csv.writer(f)
        #     writer.writerow([param, f'{fid:g}'])

    torch.distributed.barrier()

def save_fid_to_csv(file_path, scaler, fid):
    """Save scaler and FID value to a CSV file."""
    ...
    # Open file in append mode, so we don't overwrite existing data
    with open(file_path, mode='a', newline='') as csv_file:
        writer = csv.writer(csv_file)
        
        # If the file does not exist, write the header row
        if not file_exists:
            writer.writerow(['Scaler', 'FID'])  # Header row (only if file doesn't exist)
        
        # Append the data row
        writer.writerow([scaler, fid])
#----------------------------------------------------------------------------
